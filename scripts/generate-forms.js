...
const tiers = ['student', 'collaborator', 'professional', 'senior', 'fellow', 'distinguished_fellow'];
const basePath = path.join(process.cwd(), 'src/app/(public)/membership');

tiers.forEach(tier => {
  const dir = path.join(basePath, tier);
  if (!fs.existsSync(dir)) {
    fs.mkdirSync(dir, { recursive: true });
  }

  const needsPublications = ['professional', 'senior', 'fellow', 'distinguished_fellow'].includes(tier);
  const needsAchievements = ['senior', 'fellow', 'distinguished_fellow'].includes(tier);
  const needsReferences = ['fellow', 'distinguished_fellow'].includes(tier);
...
export default function ${tier.replace(/_([a-z])/g, g => g[1].toUpperCase()).replace(/^[a-z]/, g => g.toUpperCase())}ApplicationPage() {
...
${needsPublications ? `
                <div className={styles.fullWidth}>
                  <Textarea
                    ...
                    rows={4}
                    {...register('publications', { required: ${tier !== 'professional' ? "'Publications are required for this tier'" : 'false'} })}
...
`;
  ...
  console.log(`Generated page for ${tier}`);
});

// Clean up the dynamic route
const dynamicRoutePath = path.join(basePath, '[tierId]');
if (fs.existsSync(dynamicRoutePath)) {
  fs.rmSync(dynamicRoutePath, { recursive: true, force: true });
}
console.log('Cleaned up dynamic route');
